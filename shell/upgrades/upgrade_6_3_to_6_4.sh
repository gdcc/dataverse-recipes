#!/bin/bash
# Used release to generate this: https://github.com/IQSS/dataverse/releases/tag/v6.4

# Get the directory where the script is located
SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &> /dev/null && pwd)

# Logging configuration
LOGFILE="$SCRIPT_DIR/dataverse_upgrade_6_3_to_6_4.log"
echo "" > "$LOGFILE"
STATE_FILE="$SCRIPT_DIR/upgrade_6_3_to_6_4.state"
CITATION_TSV_URL="https://raw.githubusercontent.com/IQSS/dataverse/v6.4/scripts/api/data/metadatablocks/citation.tsv"
SOLR_SCHEMA_URL="https://raw.githubusercontent.com/IQSS/dataverse/v6.4/conf/solr/schema.xml"
UPDATE_FIELDS_URL="https://raw.githubusercontent.com/IQSS/dataverse/v6.4/conf/solr/update-fields.sh"
TARGET_VERSION="6.4"
CURRENT_VERSION="6.3"
...
DATAVERSE_WAR_HASH="1a5233ce4157f2299d1a661ea2d192196a506642"
...
deploy_dataverse() {
    ...
    log "Downloading Dataverse 6.4 WAR file to $DEPLOY_DIR..."
...
update_solr_schema() {
    ...
    # Download standard v6.4 schema.xml
    log "Downloading standard v6.4 schema.xml..."
...
reindex_solr() {
    ...
    read -p "Do you want to reindex Solr? This is recommended for the schema updates. (y/n): " DO_REINDEX
...
run_reexport_all() {
    ...
    log "This is necessary because of changes to the Datacite and oai_dc export formats."
...
update_datacite_metadata() {
    ...
    read -p "Do you use DataCite as your PID provider? (y/n): " USE_DATACITE
    
    if [[ "$USE_DATACITE" =~ ^[Yy]$ ]]; then
        log "Updating DataCite metadata for all published datasets..."
        log "This will send enhanced metadata including relation types to DataCite."
        ...
        if [[ "$CONFIRM_DATACITE" =~ ^[Yy]$ ]]; then
            ...
            curl http://localhost:8080/api/datasets/modifyRegistrationPIDMetadataAll
...
# Function to fix broken thumbnails (optional)
fix_broken_thumbnails() {
    log "Fixing broken thumbnails is optional and only needed if you experienced the thumbnail bug..."
    read -p "Do you want to clear thumbnail failure flags to restore broken thumbnails? (y/n): " FIX_THUMBNAILS
    
    if [[ "$FIX_THUMBNAILS" =~ ^[Yy]$ ]]; then
        log "Clearing thumbnail failure flags..."
        curl -s -f http://localhost:8080/api/admin/clearThumbnailFailureFlag || return 1
        log "Thumbnail failure flags cleared. Thumbnails should regenerate automatically."
    else
        log "Skipping thumbnail fix."
    fi
...
main() {
    ...
    # STEP 11: Update DataCite metadata (optional)
    ...
    # STEP 12: Fix broken thumbnails (optional)
    if ! is_step_completed "THUMBNAILS_FIXED"; then
        fix_broken_thumbnails
        mark_step_as_complete "THUMBNAILS_FIXED"
    else
        log "Step 'THUMBNAILS_FIXED' already completed. Skipping."
    fi
    ...
    log "1. If you use PermaLinks with a custom base-url, you must manually append '/citation?persistentId=' to the base URL."
    log "2. Enhanced DataCite metadata is now being sent automatically for new dataset publications."
    log "3. File DOIs are now reserved during upload (not at publication time) if using DataCite."
    log "4. Check the server logs for any issues with the reExportAll or DataCite updates."
    log ""
